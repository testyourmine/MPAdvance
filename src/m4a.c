...
{
    if (mplayInfo->lockStatus == ID_NUMBER && ++mplayInfo->lockStatus) {
...
static inline int CgbPan(struct MixerSource *chan, u8 rightVol, u8 leftVol)
{
    if (rightVol >= leftVol) {
...
void CgbSound(void)
{
    ...
    for (ch = 1, channels = soundInfo->cgbChans, sp1C = &sp00; ch <= 4; ch++, channels++) {
        ...
        } else if ((channels->status & SOUND_CHANNEL_SF_IEC) || (!((REG_NR52 >> *sp1C) & 1))) {
    ...
    envelope_complete:
        ...
        if (channels->data.cgb.cgbStatus & CGB_CHANNEL_MO_PIT) {
            ...
            channels->data.cgb.nrx4 = (channels->data.cgb.nrx4 & 0xC0) + ((channels->data.cgb.freq & 0x3F00) >> 8);
